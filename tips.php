...
//Tipps und Tricks Auflistung 
elseif($_REQUEST['action'] === "overview")
{
	$sql_tt_cat	= "SELECT * FROM categories WHERE id = " . $_GET['cat'];
	$res_tt_cat	= mysql_query($sql_tt_cat) or die(__FILE__.__LINE__.mysql_error());
...
<?php
	...
	//Neues Blätterfunktion initialisieren
	$pagescroll	= new Pagescroll(MAX_TIPS_PER_PAGE, $start);
	$pagescroll->getNumberOfPosts("tips", "cat = '" . $_GET['cat'] . "'");
	
	$pagescroll->setLinkFormat("?site=tips&action=overview&cat=" . $_GET['cat'], "pagescroll_link", "pagescroll_link_active");
	//Ausgabe Blätterfunktion
	try
	{
		$pagescroll->getQueryLimit();
	...
	$res_tt_po	= mysql_query($sql_tt_po) or die(__FILE__.__LINE__.mysql_error());
	$num_tt_po	= mysql_num_rows($res_tt_po);
...
elseif ($_REQUEST['action'] === "detailview")
{
	...
	if ($num_tt_po > 0)
	{
		...
		$res_upd	= mysql_query($sql_upd) or $notify->setNotificationType("warning").$notify->addMessage("SQL-Fehler in Datei: " . __FILE__ . "<br /><br />" . mysql_error());
