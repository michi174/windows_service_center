...
{	
	...
	/**
	 * Variablen, die durch Funktionen ersetzt werden m�ssen
	 *
	...
	protected function parseIf($section)
	{
		...
		while(preg_match($pattern, $section, $matches) == true)
		{
			...
			//�berpr�fen, ob noch nicht ersetzte Variablen vorhanden sind.
				...
				if($datarow_db_num_rows > 0)
				{
					...
					while(($datarow_dataset	= $datarow_db_ressource->fetch_assoc()) == true)
					{
						...
						foreach($subrow_names[$datarow_name] as $subrow_key => $subrow_name)
						{
							...
							{	
								//SQL-Befehl f�r Subrow kompilieren.
								foreach($this->datarow_vars[$datarow_name] as $datarow_var => $datarow_var_inhalt)
								{		
								...
								//Datens�tze der Subrow auslesen
								...
								if($subrow_db_num_rows > 0)
								{
									...
									{
										//Jetzt m�ssen die Subrow Template Variablen durch die Werte ersetzt werden...
	...
	/**
	 * Ver�ndert den Inhalt der Datarow Variablen mithilfe der �bergeben Funktion.
	 *
